...
describe("DesktopSettingsStateQuery", () => {
  it("reads agent status in one batched session call", async () => {
    ...
    const session = {
      ...
      getAgentStatus() {
        ...
        throw new Error("Desktop settings query should not call getAgentStatus per agent");
      },
      ...
      getAgentConnectionModel() {
        return null;
      },
      listSavedConnectionsForAgent() {
        return [];
      },
      getLatestRollbackableMutation() {
        return null;
      },
    ...
    const query = new DesktopSettingsStateQuery(
      {
        environment: EnvironmentSource.empty(),
        resolveDesktopAgentHome: (agentId) => `/tmp/${agentId}`,
        resolveDefaultDesktopAgentHome: (agentId) => `/tmp/default/${agentId}`,
      },
    ...
    const state = await query.read(session, { refreshUsage: false });
