...
import { AccessRegistry, type AccessRecord } from "@nile/core/models/access";
import { EndpointRegistry } from "@nile/core/models/endpoint";
import { SqliteAccessStore } from "../../../core/src/models/access/SqliteAccessStore";
import type { GatewayProbeResult } from "@nile/connections/setup";
...
describe("ConnectionCreator", () => {
  ...
  test("stores gateway endpoints with every detected supported protocol", async () => {
    ...
    try {
      ...
      expect(accessRegistry.list()[0]?.enabledAgents).toEqual(["codex", "claude", "openclaw", "opencode"]);
    } finally {
      database.close();
    }
  });
  ...
  test("upgrades an existing gateway endpoint in place when probing finds more protocols", async () => {
    ...
    try {
      ...
      expect(accessRegistry.get("frank")?.enabledAgents).toEqual(["codex", "claude", "openclaw", "opencode"]);
