...
export class SqliteAccessStore implements AccessStore {
  ...
  private initialize(): void {
    ...
    if (!this.hasColumn("accesses", "openclaw_model_id")) {
      this.database.exec("ALTER TABLE accesses ADD COLUMN openclaw_model_id TEXT");
    }
    if (!this.hasColumn("accesses", "api_key_source")) {
      this.database.exec("ALTER TABLE accesses ADD COLUMN api_key_source TEXT");
    }
    if (!this.hasColumn("accesses", "env_key")) {
      this.database.exec("ALTER TABLE accesses ADD COLUMN env_key TEXT");
    }
  }
