...
import type { DetectedAgentState, ImportCurrentConnectionResult } from "../../runtime-local/AgentAdapterTypes";
...
export class AgentImportSupport {
  constructor(
  ...
  private matchesOpenClawModel(candidate: AgentImportCandidate, accessId: string): boolean {
    const current = this.accessRegistry.get(accessId);
    return (candidate.access.openclawModelId?.trim() || undefined)
      === (current?.openclawModelId?.trim() || undefined);
  }
}
