...
import type { ConnectionIdentityResolver } from "@nile/core/models/connection";
...
export class LiveSetupFactory {
  constructor(
    private readonly identityKeyResolver: ConnectionIdentityResolver,
  ) {}
  ...
  buildOpenAiSessionProfileState(credential: {
      accessToken: string;
      refreshToken: string;
      expiresAt?: number;
      accountId?: string;
    ...
    const labelHint = email || "OpenAI Session";
