...
import type { OpenClawProjection } from "./ProjectionTypes";

class OpenClawProjectionStrategy {
  resolve(input: ProjectionInput): OpenClawProjection {
    const modelId = input.modelId?.trim();
    if (!modelId) {
      throw new AgentProjectionError("OpenClaw requires a saved modelId on the selected connection");
    }
    ...
    throw new AgentProjectionError(`Endpoint ${input.endpoint.id} does not support an OpenClaw-compatible protocol`);
  }
...
const STRATEGY = new OpenClawProjectionStrategy();

export const OPENCLAW_PROJECTION = {
  agentId: "openclaw",
