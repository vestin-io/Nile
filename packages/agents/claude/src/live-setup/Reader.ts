...
import { ConnectionNaming } from "@nile/core/models/connection";
...
import type { ReadLiveSetupResult, ResolvedLiveState } from "./Internal";
...
export class LiveSetupReader {
  ...
  private resolveLiveState(
    ...
    const resolvedState: ResolvedLiveState = {
      ...(this.settingsStore.readModel() ? { modelId: this.settingsStore.readModel()! } : {}),
      endpoint,
      access: {
