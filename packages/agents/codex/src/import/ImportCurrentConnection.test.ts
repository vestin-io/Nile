...
describe("ImportCurrentConnection", () => {
  ...
  it("canonicalizes an existing OpenClaw-only OpenAI session into a standard OpenAI session", async () => {
    const setup = createSetup({
      authFile: openAiAuthFile("work@example.com"),
    });
    seedEndpoint(setup.dbPath, {
    ...
    const result = await importer.importCurrent();
    const migratedRegistry = AccessRegistry.open(setup.dbPath, setup.credentialStore);
    const migrated = migratedRegistry.get("legacy-openclaw-session");
